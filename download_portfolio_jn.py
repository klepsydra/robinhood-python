...
def download_portfolio(cache_mode):
  with open('portfolio.csv', 'w', newline='') as csv_file:
    ...
    for position in portfolio.positions_by_equity_worth:
        stock = Stock(position['symbol'])
        position['sector'] = stock.get_sector()
        if not position['sector']:
            # TODO: if stock.get_company()['issueType'] is 'cs' {common stock} https://iextrading.com/developer/docs/#company
            # call https://www.barchart.com/ondemand/api/getETFDetails
            position['sector'] = 'ETF'
        position['pe_ratio'] = stock.get_pe_ratio()
